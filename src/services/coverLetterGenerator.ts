...
async function getLanguageEntry(
  cvLanguage: string,
): Promise<LangEntry | undefined> {
  const row = await prisma.settings.findUnique({
    where: { key: 'general_settings' },
  });
  if (!row) return undefined;
  const languages: LangEntry[] =
    (JSON.parse(row.value) as { languages?: LangEntry[] }).languages ?? [];
...
const TEMPLATE_PATH = path.resolve(
  process.cwd(),
  'src/templates/cover_letter.html',
);
...
export async function generateCoverLetter(
  ...
  const linkedinUrl = basic_info.linkedin || null;
  const githubUrl = basic_info.github || null;
...
- Keep each paragraph to 3-5 sentences (paragraph 4 is always 1 sentence)
  ...
  if (user?.show_agent_info_in_cv) {
    ...
    if (agentClient) {
      ...
      const agentPhone = agent.phone
        ? ` · ${esc(formatPhone(agent.phone))}`
        : '';
      const agentFirstName =
        isPl && agent.first_name_genitive
          ? agent.first_name_genitive
          : agent.first_name;
      const agentLastName =
        isPl && agent.last_name_genitive
          ? agent.last_name_genitive
          : agent.last_name;
