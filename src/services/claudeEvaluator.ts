      ...
      evaluations: {
        ...
        items: {
          ...
          properties: {
            ...
            matched_reasons: {
              type: 'object',
              description: 'CRITICAL: No salary. No work model. No generic seniority match. No skills already in missing_skills. Only reference data explicitly present in the offer.',
              properties: {
                pros: { type: 'array', items: { type: 'string' }, description: 'List up to 3 genuine advantages for this candidate. Focus on: specific skill matches that differentiate (e.g. GraphQL when it\'s required), domain/industry fit, leadership or architecture scope, step-up opportunity. Skip: salary, work model, generic seniority match.' },
                cons: { type: 'array', items: { type: 'string' }, description: 'List up to 3 genuine concerns. Focus on: role scope mismatch (fullstack vs frontend target), missing primary skills not already in missing_skills, domain gap (mobile/IoT when targeting web), seniority ambiguity. Skip: salary, work model, skills already in missing_skills array.' },
              },
              required: ['pros', 'cons'],
            },
