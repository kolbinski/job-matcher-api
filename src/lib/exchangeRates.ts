...
export async function updateExchangeRates(): Promise<void> {
  ...
  const generalSetting = await prisma.settings.findUnique({
  ...
  try {
    ...
    console.warn(
      '[exchangeRates] failed to parse general_settings currencies, using fallback list',
    );
  }
  ...
  try {
    ...
    console.warn(
      '[exchangeRates] rate fetch failed, using fallback values:',
      err,
    );
  }
