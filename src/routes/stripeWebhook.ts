...
stripeWebhookRouter.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      console.log(
        '[stripe-webhook] customer field raw:',
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      if (
        sessionType === 'scan_package' ||
        sessionType === 'cv_package' ||
        sessionType === 'cl_package' ||
        sessionType === 'profile_rematch_package' ||
        sessionType === 'review_package'
      ) {
        const userId = obj.metadata?.['user_id'];
        const amount = Number(obj.metadata?.['amount'] ?? '0');
        if (!userId) {
          console.error(
            `[stripe-webhook] ${sessionType}: missing user_id in metadata`,
          );
        ...
        if (sessionType === 'profile_rematch_package') {
          ...
          console.log(
            `[stripe-webhook] profile_rematch_package: incremented profile_relevant_change_counter_max by ${amount} for user ${userId}`,
          );
          ...
          console.log(
            `[stripe-webhook] review_package: incremented review_by_ai_counter_max by ${amount} for user ${userId}`,
          );
        } else {
          const counterField =
            sessionType === 'cv_package'
          ...
          console.log(
            `[stripe-webhook] ${sessionType}: incremented ${counterField} by ${amount} for user ${userId}`,
          );
        }
        ...
        console.log(
          `[stripe-webhook] ${sessionType}: retrieved customer from session: ${packageCustomerId}`,
        );
        ...
        if (packageCustomerId) {
          ...
          console.log(
            `[stripe-webhook] saved stripe_customer_id: ${packageCustomerId} for user: ${userId}`,
          );
        }
      ...
      console.log('[stripe-webhook] checkout.session.completed fields:', {
      ...
      if (!userId || !stripeSubscriptionId) {
        console.error(
          '[stripe-webhook] Missing userId or stripeSubscriptionId in checkout.session.completed',
        );
        return res.json({ received: true });
      }
      ...
      const item0 = stripeSub.items.data[0] as unknown as {
      ...
      const proLimits = proPlan.limits as {
      ...
      await prisma.user.update({
        ...
        data: {
          scan_page_counter_max: {
            increment:
              (proLimits.max_scan_page ?? 0) - (freeLimits.max_scan_page ?? 0),
          },
          cv_counter_max: {
            increment: (proLimits.max_cv ?? 0) - (freeLimits.max_cv ?? 0),
          },
          cl_counter_max: {
            increment: (proLimits.max_cl ?? 0) - (freeLimits.max_cl ?? 0),
          },
          review_by_ai_counter_max: {
            increment:
              (proLimits.max_review_by_ai ?? 0) -
              (freeLimits.max_review_by_ai ?? 0),
          },
          status_change_counter_max: null,
        },
      });
    ...
    if (event.type === 'customer.subscription.deleted') {
      ...
      if (!sub) {
        console.error(
          `[stripe-webhook] No subscription found for stripe_subscription_id: ${obj.id}`,
        );
        return res.json({ received: true });
      }
      ...
      console.log(
        `[stripe-webhook] Downgraded user ${sub.user_id} to Free plan`,
      );

      await prisma.user.update({
        where: { id: sub.user_id },
        data: {
          status_change_counter_max: (freePlan.limits as { max_status_change?: number | null } | null)?.max_status_change ?? null,
        },
      });
    }
    ...
    const isSignatureError =
      message.includes('No signatures found') || message.includes('signature');
    if (isSignatureError) {
